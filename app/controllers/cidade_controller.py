...
class Cidade_Controller:
    ...
    def update(self):
        try:
            if self.cidade_selecionado is None:
                ...
                return
            nome, estado = self.view.ler_dados_cidade()
            self.cidade_selecionado.atualizar_dados(nome, estado)
            self.dao.update(self.cidade_selecionado)
            self.get_all()
    ...
    def delete(self):
        if self.cidade_selecionado is None:
            self.view.exibir_mensagem("Selecione uma cidade na lista.", False)
            return
        if self.view.comfirmar_exclusao():
            return
        try:
            sucesso = self.dao.delete(self.cidade_selecionado.id)
            if sucesso:
                self.cidade_selecionado = None
            ...
            self.view.exibir_mensagem("Problema ao excluir a cidade ", False)
